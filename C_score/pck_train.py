...
def normalize_feats(args, feat, epsilon=1e-10):
...
def prepare_feature_paths_and_load(aggre_net, img_path, flip, ensemble, num_patches, device, model):
    ...
    path = f"{feature_base}_{model}{suffix_flip}.pt".replace('features', ensemble_folder)
    desc = torch.load(path).to(device)
    desc = aggre_net(desc).reshape(1, 1, -1, num_patches**2).permute(0, 1, 3, 2)
...
def get_patch_descriptors(args, aggre_net, num_patches, files, pair_idx, flip=False, flip2=False, img1=None, img2=None, device='cuda'):
    ...
    img1_desc, mask1 = prepare_feature_paths_and_load(aggre_net, img_path_1, flip, args.ENSEMBLE, num_patches, device, args.MODEL)
    img2_desc, mask2 = prepare_feature_paths_and_load(aggre_net, img_path_2, flip2, args.ENSEMBLE, num_patches, device, args.MODEL)
    # normalize the desc
