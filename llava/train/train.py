...
import os
os.environ["WANDB_PROJECT"] = "diffllava_tuning"
import copy
...
dummy_feature_map={"runwayml/stable-diffusion-v1-5_feature": (576, 1280)}
...
class ModelArguments:
    ...
    up_ft_index: int = field(default=0)
    t: int = field(default=1)
    prompt: str = field(default='')
    ensemble_size: int = field(default=1)
    img_size: int = field(default=768)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            # image = Image.open("/opt/tiger/LLaVA1.5/cat.jpeg").convert('RGB')
...
class LazyFeatureDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            image_file = self.list_data_dict[i]['image']
            image_file = image_file.split('.')[0]+'.pt'
            image_folder = self.data_args.image_folder
            # mem leak issue 
        ...
        # image exist in the data
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = torch.load(os.path.join(image_folder, image_file))
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            data_dict['image'] = torch.zeros(dummy_feature_map[self.data_args.feature])
        return data_dict
...
def make_feature_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        if 'feature' not in model_args.vision_tower:
            vision_tower = model.get_vision_tower()
            if type(vision_tower) is nn.ModuleList:
                for v in vision_tower:
                    if v:
                        v.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
            else:
            ...
            if type(vision_tower) is nn.ModuleList:
                ...
                for v in vision_tower:
                    if v is None:
                        data_args.image_processor.append(None)
                    else:
                        data_args.image_processor.append(v.image_processor)
            else:
            ...
            data_args.feature = model_args.vision_tower
        ...
        if model_args.tune_mm_mlp_adapter:
            ...
            if type(projector) is nn.ModuleList:
                for p in projector:
                    if p:
        ...
        # MOF not avaliable for low bit training
        if training_args.bits in [4, 8]:
        ...
        if 'diffusion' in model_args.vision_tower or 'sd' in model_args.vision_tower or 'DiT' in model_args.vision_tower:
            model.config.vision_tower = model_args.vision_tower
            model.config.up_ft_index = model_args.up_ft_index
            model.config.t = model_args.t
            model.config.prompt = model_args.prompt
            model.config.ensemble_size = model_args.ensemble_size
    ...
    if 'feature' not in model_args.vision_tower:
        ...
        data_module = make_feature_data_module(tokenizer=tokenizer,
                                                data_args=data_args)
