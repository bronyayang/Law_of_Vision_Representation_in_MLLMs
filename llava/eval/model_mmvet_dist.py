...
class CustomDataset(Dataset):
    def __init__(self, questions, tokenizer, image_processor, model_config, mm_vet=None):
    ...
    def __getitem__(self, index):
        ...
        image_file = os.path.join(self.mm_vet_path, line["imagename"])
        qs = line["question"]
...
def eval_model():
    ...
    generation_config = GenerationConfig.from_pretrained('/opt/tiger/LLaVA1.5/ckpts/llava-v1.5-7b')
    ...
    data = [value for key, value in sorted(data_dict.items(), key=lambda item: int(item[0].split('_')[1]))]
    data_keys = [key for key, value in sorted(data_dict.items(), key=lambda item: int(item[0].split('_')[1]))]
    ...
    test_dataset = CustomDataset(data, tokenizer, image_processor, model.config, mm_vet=data_args.image_folder)
