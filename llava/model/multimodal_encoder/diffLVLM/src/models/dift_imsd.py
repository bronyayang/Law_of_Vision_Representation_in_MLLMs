...
class OneStepSDPipeline(StableDiffusionImageVariationPipeline):
    @torch.no_grad()
    def __call__(
        self,
        img_tensor,
...
class IMSDFeaturizer:
    def __init__(self, sd_id='lambdalabs/sd-image-variations-diffusers'):
    ...
    def forward(self, img_tensor, prompt, t=1, up_ft_index=0, ensemble_size=1):
        ...
        prompt = F.interpolate(img_tensor, size=(224, 224), mode='bilinear')
        prompt = prompt.bfloat16()
        img_tensor = img_tensor.bfloat16()
        prompt_embeds = self.pipe._encode_image(prompt, 
                                                device='cuda', 
